...
class Keywords {
	...
	private function my_ucwords($str, $is_name=false) {
	   ...
	   $str = preg_replace_callback('/\\b(\\w)/m', function($matches) { return strtoupper("\0"); }, strtolower(trim($str)));

	   if ($all_uppercase) {
	       // capitalize acronymns and initialisms e.g. PHP
	       $str = preg_replace_callback("/\\b($all_uppercase)\\b/m", function($matches) { return strtoupper("\0"); }, $str);
	   }
	   if ($all_lowercase) {
	       // decapitalize short words e.g. and
	       if ($is_name) {
	           // all occurences will be changed to lowercase
	           $str = preg_replace_callback("/\\b($all_lowercase)\\b/m", function($matches) { return strtolower("\0"); }, $str);
	       } else {
	           // first and last word will not be changed to lower case (i.e. titles)
	           $str = preg_replace_callback("/(?<=\\W)($all_lowercase)(?=\\W)/m", function($matches) { return strtolower("\0"); }, $str);
	       }
	   }
	   if ($prefixes) {
	       // capitalize letter after certain name prefixes e.g 'Mc'
	       $str = preg_replace_callback("/\\b($prefixes)(\\w)/m", function($matches) { return "\0".strtoupper("\1"); }, $str);
	   }
	   if ($suffixes) {
	       // decapitalize certain word suffixes e.g. 's
	       $str = preg_replace_callback("/(\\w)($suffixes)\\b/m",  function($matches) { return "\0".strtolower("\1"); }, $str);
	   }
	   return $str;
	}
